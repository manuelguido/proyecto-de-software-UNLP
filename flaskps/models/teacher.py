class Teacher(object):
    ...
    def all(cls):
        ...
        sql = """
            ...
            INNER JOIN locations ON teachers.location_id = location.location_id
            INNER JOIN document_types ON teachers.document_type_id = document_types.document_type_id
        """
        cursor.execute(sql)
    ...
    def get(cls, id_data):
        sql = """
            ...
            FROM teachers
            INNER JOIN genders ON teachers.gender_id = genders.gender_id
            INNER JOIN locations ON teachers.location_id = locations.location_id
            INNER JOIN document_types ON teachers.document_type_id = document_types.document_type_id
    ...
    def create(cls, data):
        sql = """
            INSERT INTO teachers (lastname, name, birth_date, location_id, address, gender_id, document_type_id, document_number, phone)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
        """
        cursor = cls.db.cursor()
        cursor.execute(sql, (data['lastname'], data['name'], data['birth_date'], data['location_id'], data['address'], data['gender_id'], data['document_type_id'], data['document_number'], data['phone']))
        cls.db.commit()
        return True
    ...
    def update(cls, data):
        ...
        cursor.execute("""
               UPDATE teachers
               SET lastname=%s, name=%s, birth_date=%s, location_id=%s, address=%s, gender_id=%s, document_type_id=%s, document_number=%s, phone=%s
               WHERE teacher_id=%s
            """, (data['lastname'], data['name'], data['birth_date'], data['location_id'], data['address'], data['gender_id'], data['document_type_id'], data['document_number'], data['phone'], data['teacher_id']))
        cls.db.commit()
        return True
    ...
    def delete(cls, id_data):
        cursor = cls.db.cursor()
        cursor.execute("DELETE FROM teachers WHERE teacher_id=%s", (id_data))
        cls.db.commit()
        return True
